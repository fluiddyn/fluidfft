...
/*    flags = FFTW_PATIENT;*/
#ifdef SINGLE_PREC
  arrayX    = (real_cu*) fftwf_malloc(sizeof(real_cu)*nX0loc * nX1);
  arrayK_pR = (myfftw_complex*) fftwf_malloc(sizeof(myfftw_complex) *
                                          nX0loc * (nKx+1));
  arrayK_pC = (myfftw_complex*) fftwf_malloc(sizeof(myfftw_complex) * nKxloc * N0);
  ...
  plan_c2c_bwd = fftwf_plan_many_dft(
      ...
      sign, flags);
#else
  arrayX    = (real_cu*) fftw_malloc(sizeof(real_cu)*nX0loc * nX1);
...
real_cu FFT2DWithFFTW1D::compute_energy_from_X(real_cu* fieldX)
{
  int ii;
  real_cu energy = 0;
...
real_cu FFT2DWithFFTW1D::compute_energy_from_K(myfftw_complex* fieldK)
{
  int i0, i1;
  real_cu energy = 0;
...
real_cu FFT2DWithFFTW1D::compute_mean_from_X(real_cu* fieldX)
{
  real_cu mean = 0.;
  int ii;
...
real_cu FFT2DWithFFTW1D::compute_mean_from_K(myfftw_complex* fieldK)
{
  real_cu mean = creal(fieldK[0]);
  return mean;
}
void FFT2DWithFFTW1D::fft(real_cu *fieldX, myfftw_complex *fieldK)
{
  int ii, jj;
...
void FFT2DWithFFTW1D::ifft(myfftw_complex *fieldK, real_cu *fieldX)
{
  int ii, jj;
...
void FFT2DWithFFTW1D::init_array_X_random(real_cu* &fieldX)
{
  int ii;
